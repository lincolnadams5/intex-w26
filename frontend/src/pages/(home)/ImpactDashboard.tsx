    ...
    return (
      <div className="min-h-screen bg-[var(--color-surface)] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-[var(--color-on-surface-variant)]">Loading impact data...</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen bg-[var(--color-surface)] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-[var(--color-on-surface-variant)] mb-6">{error}</p>
      ...
      <main className="max-w-[1200px] mx-auto px-6 py-12">
        ...
        {summary && (
          <section className="mb-12">
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              <div className="col-span-2 bg-gradient-to-br from-[var(--color-primary)] to-[var(--color-primary-container)] text-white rounded-xl p-6">
              ...
              {donationsByType.map((item) => {
                ...
                return (
                  <div key={item.donationType} className="flex items-center gap-4">
                    <span className="w-24 text-sm text-[var(--color-on-surface-variant)] shrink-0">{formatDonationType(item.donationType)}</span>
                    <div className="flex-1 h-6 bg-[var(--color-surface-container-high)] rounded overflow-hidden">
                      <div className="h-full bg-[var(--color-primary)] rounded transition-all" style={{ width: `${percentage}%` }}></div>
                    </div>
                ...
                return (
                  <div key={item.programArea}>
                    <div className="flex justify-between mb-1">
                      <span className="text-sm text-[var(--color-on-surface)]">{item.programArea}</span>
                      <span className="text-sm font-semibold">{formatCurrency(item.totalAllocated)}</span>
                    </div>
                    <div className="h-3 bg-[var(--color-surface-container-high)] rounded overflow-hidden">
                      <div className="h-full bg-[var(--color-secondary)] rounded transition-all" style={{ width: `${percentage}%` }}></div>
                    </div>
                    <span className="text-xs text-[var(--color-on-surface-variant)]">{percentage.toFixed(1)}%</span>
                  </div>
                )
              })}
            </div>
          ...
          <section className="bg-[var(--surface-container-lowest)] rounded-xl p-6 shadow-[var(--shadow-ambient)]">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {supportersByType.map((item) => (
                <div key={item.supporterType} className="bg-[var(--color-surface-container-low)] rounded-lg p-4 text-center">
                  <span className="block text-2xl font-bold text-[var(--color-primary)]">{formatNumber(item.count)}</span>
                  <span className="block text-xs text-[var(--color-on-surface)] mt-1">{formatSupporterType(item.supporterType)}</span>
                  <span className="block text-xs text-[var(--color-on-surface-variant)] mt-1">{item.activeCount} active</span>
                </div>
              ))}
            </div>
          </section>
            ...
            {donationsByMonth.slice(-12).map((item) => {
              ...
              return (
                <div key={`${item.year}-${item.month}`} className="flex-1 flex flex-col items-center gap-2">
                  ...
                  <span className="text-xs text-[var(--color-on-surface-variant)]">{getMonthName(item.month)}</span>
                </div>
              )
            })}
          </div>
          ...
          <section className="bg-[var(--surface-container-lowest)] rounded-xl p-6 shadow-[var(--shadow-ambient)] mb-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {campaigns.map((campaign) => (
                <div key={campaign.campaignName} className="bg-[var(--color-surface-container-low)] rounded-lg p-5">
                  <h4 className="text-base mb-4">{campaign.campaignName}</h4>
                  <div className="grid grid-cols-3 gap-2 text-center">
                    <div>
                      <span className="block text-lg font-bold text-[var(--color-primary)]">{formatCurrency(campaign.totalRaised)}</span>
                      <span className="text-xs text-[var(--color-on-surface-variant)]">Raised</span>
                    </div>
                    <div>
                      <span className="block text-lg font-bold text-[var(--color-on-surface)]">{campaign.uniqueDonors}</span>
                      <span className="text-xs text-[var(--color-on-surface-variant)]">Donors</span>
                    </div>
                    <div>
                      <span className="block text-lg font-bold text-[var(--color-on-surface)]">{campaign.donationCount}</span>
                      <span className="text-xs text-[var(--color-on-surface-variant)]">Gifts</span>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <section className="bg-gradient-to-br from-[var(--color-primary)] to-[var(--color-primary-container)] text-white rounded-xl p-10 text-center">
          ...
          <div className="flex gap-4 justify-center flex-wrap">
            <Link to="/donate" className="btn btn-large bg-white text-[var(--color-primary)] hover:bg-white/90">Donate Now</Link>
